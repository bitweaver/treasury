...
$query = "
	...
	WHERE la.`attachment_plugin_guid` = ?";
if( $ret = $gBitSystem->mDb->getAll( $query, array( PLUGIN_GUID_TREASURY_FILE ))) {
	foreach( $ret as $update ) {
		if( LibertyAttachable::storePrimaryAttachmentId( $update )) {
			echo ">>>> Updated Treasury file: {$update['storage_path']}\n";
			$gBitSystem->mDb->query( "DELETE FROM `".BIT_DB_PREFIX."liberty_attachments_map` WHERE `content_id`=? AND `attachment_id`", array( $update['content_id'], $update['attachment_id'] ));
		}
	}
}
echo "</pre>";
